...
const Roster: React.FC = () => {
  ...
  const handleDeletePlayer = async (idToDelete: string) => {
    if (!appData) return;
    const player = appData.players.find(p => p.id === idToDelete);
    if (!player) return;

    const statsCount = appData.stats.filter(s => s.playerId === idToDelete).length;
    ...
    await deletePlayer(idToDelete); // This will also handle stats deletion via DB cascade
    if (editingPlayerId === idToDelete) cancelEditPlayer();
  };
  ...
  const handleSavePlayer = async () => {
    ...
    const isDuplicate = appData.players.some(p => p.teamId === playerForm.teamId && p.number === playerForm.number && p.id !== editingPlayerId);
    ...
    if (editingPlayerId === 'new') {
        ...
        await updatePlayer(editingPlayerId, playerForm);
    }
    cancelEditPlayer();
  };
  ...
  const handleDeleteTeam = async () => {
      if (!appData || !editingTeamId || editingTeamId === 'new') return;
      const team = appData.teams.find(t => t.id === editingTeamId);
      if (!team) return;
      // Database cascades should handle deleting players, matches, stats.
      // A confirmation is still good practice.
      if (!window.confirm(`Are you sure you want to delete team "${team.name}"? This will also delete all associated players, matches, and stats.`)) return;
      await deleteTeam(editingTeamId);
      cancelEditTeam();
  };

  const handleSaveTeam = async () => {
    if (!appData || !teamForm.name) return;
    
    let teamPayload = { ...teamForm };
    if (teamForm.isMain) {
        // Unset other main teams if this one is set to main
        const otherMainTeam = appData.teams.find(t => t.isMain && t.id !== editingTeamId);
        if (otherMainTeam) {
            await updateTeam(otherMainTeam.id, { isMain: false });
        }
    }

    if (editingTeamId === 'new') {
        await addTeam(teamPayload);
    } else {
        await updateTeam(editingTeamId, teamPayload);
    }
    cancelEditTeam();
  };

  const handleAddLeague = async () => {
    if (!appData || !newLeague.name) return;
    await addLeague(newLeague);
    setNewLeague({ ...newLeague, name: '' });
  };

  const handleDeleteLeague = (id: string) => {
    if (!window.confirm("Delete league? This will also remove it from any teams and delete associated matches.")) return;
    deleteLeague(id);
  };
