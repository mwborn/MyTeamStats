      ...
      <h1 className="text-2xl font-bold text-slate-800 dark:text-sky-400 no-print">Game Dashboard</h1>
       ...
       <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-wrap gap-4 items-end no-print">
          <div>
             ...
             <select className="px-3 py-2 border dark:border-slate-600 rounded-lg text-sm min-w-[200px] bg-white dark:bg-slate-700" value={selectedLeagueId} onChange={e => { setSelectedLeagueId(e.target.value); setSelectedMatchId(''); }}>
          ...
          <div>
             ...
             <select className="px-3 py-2 border dark:border-slate-600 rounded-lg text-sm min-w-[200px] bg-white dark:bg-slate-700" value={selectedMatchId} onChange={e => setSelectedMatchId(e.target.value)}>
          ...
          {currentMatch && (
              <div className="bg-slate-100 dark:bg-slate-700 p-1 rounded-lg flex items-center">
                  <button className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewTeamId === homeTeam?.id ? 'bg-white dark:bg-slate-600 shadow text-black dark:text-white' : 'text-slate-500 dark:text-slate-300 hover:text-slate-800'}`} onClick={() => setViewTeamId(homeTeam?.id || '')}> {homeTeam?.name} (Home) </button>
                  <button className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewTeamId === awayTeam?.id ? 'bg-white dark:bg-slate-600 shadow text-black dark:text-white' : 'text-slate-500 dark:text-slate-300 hover:text-slate-800'}`} onClick={() => setViewTeamId(awayTeam?.id || '')}> {awayTeam?.name} (Away) </button>
              </div>
          )}
         ...
         <div className="print-full bg-white text-black font-sans shadow-lg overflow-hidden max-w-[1200px] mx-auto relative">
                ...
                {currentMatch.quarters && (
                    <div className="absolute top-4 right-4 text-xs hidden md:block">
                       <table className="border-collapse text-center">
                          <thead><tr className="text-slate-500"><th>T</th><th>1</th><th>2</th><th>3</th><th>4</th>{(currentMatch.quarters.home[4] > 0 || currentMatch.quarters.away[4] > 0) && <th>OT1</th>}{(currentMatch.quarters.home[5] > 0 || currentMatch.quarters.away[5] > 0) && <th>OT2</th>}</tr></thead>
                          <tbody><tr><td className="pr-2 text-white font-bold border-r border-slate-800 text-right">In</td>{currentMatch.quarters.home.slice(0, 6).map((score, i) => { if (i >= 4 && score === 0 && currentMatch.quarters.away[i] === 0) return null; return <td key={i} className="px-1">{score}</td> })}</tr><tr><td className="pr-2 text-white font-bold border-r border-slate-800 text-right">Out</td>{currentMatch.quarters.away.slice(0, 6).map((score, i) => { if (i >= 4 && score === 0 && currentMatch.quarters.home[i] === 0) return null; return <td key={i} className="px-1">{score}</td> })}</tr></tbody>
                       </table>
                    </div>
                )}
            </div>
            ...
            <div className="overflow-x-auto">
               ...
               <table className="w-full text-xs border-collapse">
                 ...
                 <tbody className="divide-y divide-slate-200">{teamStats.map((s, idx) => { const p = data.players.find(pl => pl.id === s.playerId); const rowClass = idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'; return (<tr key={s.playerId} className={`${rowClass} hover:bg-yellow-50 text-center`}><td className={`p-1 text-left font-bold sticky left-0 ${rowClass}`}><span className="inline-block w-4 mr-1">{p?.number}</span> {p?.name}</td><td className="p-1 font-mono">{s.minutes.split(':')[1] || s.minutes}</td><td className="p-1">{s.twoPtMade}</td><td className="p-1">{s.twoPtAtt}</td><td className="p-1 font-mono text-slate-500">{getPct(s.twoPtMade, s.twoPtAtt)}</td><td className="p-1">{s.threePtMade}</td><td className="p-1">{s.threePtAtt}</td><td className="p-1 font-mono text-slate-500">{getPct(s.threePtMade, s.threePtAtt)}</td><td className="p-1">{s.ftMade}</td><td className="p-1">{s.ftAtt}</td><td className="p-1 font-mono text-slate-500">{getPct(s.ftMade, s.ftAtt)}</td><td className="p-1 text-slate-500">{s.rebDef}</td><td className="p-1 text-slate-500">{s.rebOff}</td><td className="p-1 font-bold">{s.rebDef + s.rebOff}</td><td className="p-1 text-slate-500">{s.foulsCommitted}</td><td className="p-1 text-slate-500">{s.foulsDrawn}</td><td className="p-1">{s.turnovers}</td><td className="p-1">{s.steals}</td><td className="p-1">{s.assists}</td><td className="p-1">{s.blocksMade}</td><td className="p-1">{s.blocksRec}</td><td className="p-1 font-bold bg-yellow-100">{s.points}</td><td className={`p-1 font-bold ${s.valuation > 0 ? 'text-green-600' : 'text-red-600'}`}>{s.valuation}</td><td className={`p-1 font-bold text-white px-1 ${s.plusMinus >= 0 ? 'bg-green-500' : 'bg-red-500'}`}>{s.plusMinus}</td></tr>);})}{<tr className="bg-[#66b64d] text-white font-bold text-center border-t-2 border-slate-400"><td className="p-2 text-left sticky left-0 bg-[#66b64d]">Grand Total</td><td className="p-1">{totals.minutes}</td><td className="p-1">{totals.twoPtMade}</td><td className="p-1">{totals.twoPtAtt}</td><td className="p-1 text-xs opacity-75">{getPct(totals.twoPtMade, totals.twoPtAtt)}</td><td className="p-1">{totals.threePtMade}</td><td className="p-1">{totals.threePtAtt}</td><td className="p-1 text-xs opacity-75">{getPct(totals.threePtMade, totals.threePtAtt)}</td><td className="p-1">{totals.ftMade}</td><td className="p-1">{totals.ftAtt}</td><td className="p-1 text-xs opacity-75">{getPct(totals.ftMade, totals.ftAtt)}</td><td className="p-1 opacity-75">{totals.rebDef}</td><td className="p-1 opacity-75">{totals.rebOff}</td><td className="p-1 text-white">{totals.rebDef + totals.rebOff}</td><td className="p-1 opacity-75">{totals.foulsCommitted}</td><td className="p-1 opacity-75">{totals.foulsDrawn}</td><td className="p-1">{totals.turnovers}</td><td className="p-1">{totals.steals}</td><td className="p-1">{totals.assists}</td><td className="p-1">{totals.blocksMade}</td><td className="p-1">{totals.blocksRec}</td><td className="p-1 text-lg">{totals.points}</td><td className="p-1">{totals.valuation}</td><td className="p-1">{totals.plusMinus}</td></tr>}</tbody>
               </table>
            </div>
